...
var _ = Describe("Testing of individual databases...", func() {
	...
	It("Check all user databases have the default extensions", func() {
		...
		for i := 0; i < len(allNodes); i++ {
			...
			for _, databaseName := range listDatabases {
				sq = ` SELECT COUNT(*) as rowCount FROM pg_extension WHERE extname IN ('pg_trgm'); `
				rowCount, err := GetRowCountUserDB(address, sq, databaseName)
				Expect(err).NotTo(HaveOccurred())
				if rowCount == 0 {
					sq = ` CREATE EXTENSION pg_trgm; `
			...
			expectedCount := 0
			if clusterService == `pgbdr` {
				sq = ` SELECT COUNT(*) as rowCount FROM pg_extension WHERE extname IN ('btree_gist', 'bdr', 'pg_stat_statements', 'uuid-ossp', 'hstore', 'pg_trgm'); `
				expectedCount = 6
			} else if clusterService == `postgresql` {
				sq = ` SELECT COUNT(*) as rowCount FROM pg_extension WHERE extname IN('btree_gist', 'pg_stat_statements', 'uuid-ossp', 'hstore', 'pg_trgm'); `
				expectedCount = 5
			}
