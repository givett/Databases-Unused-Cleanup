...
func (r *RDPG) InitSchema() (err error) {
	...
	newDefaultConfig := tasks.DefaultConfig{Key: `BackupsPath`, ClusterID: ClusterID, Value: `/var/vcap/store/pgbdr/backups`}
	newDefaultConfig.Add()

	newDefaultConfig = tasks.DefaultConfig{Key: `BackupPort`, ClusterID: ClusterID, Value: `7432`}
	newDefaultConfig.Add()

	if globals.ClusterService == "pgbdr" {
		newDefaultConfig = tasks.DefaultConfig{Key: `pgDumpBinaryLocation`, ClusterID: ClusterID, Value: `/var/vcap/packages/pgbdr/bin/pg_dump`}
		newDefaultConfig.Add()
	} else {
		newDefaultConfig = tasks.DefaultConfig{Key: `pgDumpBinaryLocation`, ClusterID: ClusterID, Value: `/var/vcap/packages/postgresql-9.4/bin/pg_dump`}
		newDefaultConfig.Add()
	}
	newDefaultConfig = tasks.DefaultConfig{Key: `defaultDaysToKeepFileHistory`, ClusterID: ClusterID, Value: `180`}
	newDefaultConfig.Add()
