	...
	for { // In case we need to wait for a precreated database on the cluster...
		...
		if err != nil {
			if err == sql.ErrNoRows {
				instanceNum := 0
				sq := fmt.Sprintf(`SELECT count(*) FROM cfsb.instances a, cfsb.plans b WHERE a.instance_id IS NULL AND a.effective_at IS NOT NULL AND a.ineffective_at IS NULL and a.decommissioned_at IS NULL AND a.cluster_service = b.cluster_service AND b.plan_id = '%s' ORDER BY a.created_at ASC LIMIT 1`, i.PlanID)
				log.Trace(fmt.Sprintf(`instances.Instance#Provision() > %s`, sq))
				err = db.Get(&instanceNum, sq)
				if err != nil {
					log.Error(fmt.Sprintf("instances.Instance#Provision(%s): Fail to query instance number > %s ! %s", i.InstanceID, sq, err))
				}
